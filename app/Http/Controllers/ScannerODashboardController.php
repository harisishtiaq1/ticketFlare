...
class ScannerODashboardController extends Controller
{
    ...
    {  
        ...
        $total_earning  = $events->pluck('bookings')->flatten()->where('status', 1)->where('is_paid', 1)->pluck('commission')->filter()->sum('organiser_earning');
        $total_bookings = $events->pluck('bookings')->flatten()->where('status', 1)->where('is_paid', 1)->sum('quantity');
        $total_scan_tickets = $events->pluck('bookings')->flatten()->where('scanner_id', Auth::id())->where('status', 1)->where('is_paid', 1)->sum('checked_in');
        ...
        return view('vendor.eventmie.o_dashboard.dashboard', compact('total_earning', 'total_events', 'total_bookings', 'eventsChart', 'total_scan_tickets'));
    }

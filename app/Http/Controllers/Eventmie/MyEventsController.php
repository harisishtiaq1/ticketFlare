...
class MyEventsController extends BaseMyEventsController
{
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            // CUSTOM
            'short_url'         => 'required|nullable|max:256',
            'currency'          => 'nullable',
    ...
    public function store_media(Request $request)
    {
        ...
        if($request->hasfile('seatingchart_image'))
        {
            ...
            $extension              = $file->getClientOriginalExtension(); // getting image extension
            $seatingchart_image     = time().rand(1,988).'.'.'webp';
            $image_resize           = Image::make($file)->encode('webp', 90)->resize(1024, 768);
            ...
            $image_resize->save(storage_path('/app/public/'.$path.$seatingchart_image));
    ...
    public function export_attendees($slug)
    {
        ...
        foreach($bookings as $key => $item)
        {
            ...
            $bookings[$key]['seat']                 = null;
            $bookings[$key]['payment_type']         = $bookings[$key]['payment_type'];
            //attendees
            if(!empty($bookings[$key]['attendees']))
            {
                ...
                if(!empty($bookings[$key]['attendees'][0]['seat_id']))
                    $bookings[$key]['seat']           = \App\Models\Seat::where(['id' => $bookings[$key]['attendees'][0]['seat_id']])->first()->name;

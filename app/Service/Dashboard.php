    ...
    public function EventTotalBySalesPrice(Request $request, $user_id = null)
    {
        ...
        if($events->isNotEmpty())
        {
            ...
            foreach ($events as $key => $event)
            {
                ...
                $data[$custom_key]['tickets_quantity'] = '<span class="badge badge-primary">'.$event->bookings->where('status', 1)->where('is_paid', 1)->sum('quantity').'</span>';
                ...
                foreach($event->tickets as $key1 => $ticket)
                {
                   if($ticket->bookings->isEmpty())
                    ...
                    $data[$custom_key]['tickets_quantity']  = $ticket->bookings->where('status', 1)->where('is_paid', 1)->sum('quantity');

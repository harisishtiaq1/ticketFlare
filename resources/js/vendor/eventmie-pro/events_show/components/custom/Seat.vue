...
export default {
    ...
    methods: {
        ...
        listenSeatEvent() {
            window.Echo.private('seatStatusChannel')
            .listenForWhisper(`seats.${this.ticket.id}.${this.booked_date_server}`, (e) => {
                console.log(e, 'seat-event');

                this.seats.every((value, index) => {

                    if(value.id == e.seat.id) {

                        // for login user
                        if(login_user_id > 0 && this.booked_date_server == e.seat.booked_date_server && login_user_id != e.seat.customer_id) {
                            this.seats[index].reserved = e.seat.is_checked > 0 ? true : false;
                        }

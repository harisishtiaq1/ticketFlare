                            ...
                            <tbody>
                                <tr  v-for="(guest, index) in guests" :key="index" >
                                    ...
                                    <td class="text-nowrap">
                                        <a href="#" class="btn btn-sm btn-danger" @click="deleteGuest(guest.id)"><i class="fas fa-trash-alt"></i> {{ trans('em.delete') }}</a>
                                    </td>

                                    <td class="text-nowrap">
                                        <a href="#" class="btn btn-sm btn-info" @click="edit_guest = 1, edit_guest_data = guest"><i class="fas fa-edit"></i> {{ trans('em.edit') }}</a>
                                    </td>

                                </tr>
                                 <tr v-if="guest.length <= 0"> <td class="text-center mt-5" colspan="3">{{ trans('em.guest_not_found')}}</td></tr>
                            </tbody>
                        </table>
                    </div>
